...
function Sidebar({ user, view, setView, onLogout, lang, collapsed, onToggleCollapse }) {
  ...
  const navItems = [
    ...
    { icon: '🌐', label: lang === 'ar' ? 'مجتمع الطلاب'    : 'Community',  id: 'community' },
...
// ─── Community View (Lark-style messages) ─────────────────────────────────
const QUICK_REACTIONS = ['❤️', '🎉', '👍', '🔥']

function communityTimeAgo(iso, lang) {
...
function CommunityView({ user, lang }) {
  ...
  async function handleReact(postId, emoji) {
    ...
    setPosts(p => p.map(x => {
      ...
      const idx = arr.indexOf(user.username)
      if (idx === -1) arr.push(user.username); else arr.splice(idx, 1)
    ...
    <div className="fade-in">
      ...
      <div style={{ background: C.surface, border: `1px solid ${C.g20}`, borderRadius: '12px', padding: '10px 14px', marginBottom: '22px' }}>
        <textarea
          ...
          placeholder={lang === 'ar' ? 'شارك نتيجتك أو سؤالك مع المجتمع...' : 'Share your result or question with the community...'}
          rows={text || image ? 3 : 1}
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px', paddingTop: '8px', borderTop: `1px solid ${C.g15}` }}>
          ...
          <button onClick={handleSubmit} disabled={posting || (!text.trim() && !image)} style={{
            ...
            {posting ? (lang === 'ar' ? '...' : '...') : (lang === 'ar' ? 'نشر' : 'Post')}
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <p style={{ color: C.silver, fontSize: '13px', textAlign: 'center', padding: '30px 0' }}>{lang === 'ar' ? 'لا توجد منشورات بعد — كن أول من يشارك!' : 'No posts yet — be the first to share!'}</p>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column' }}>
          {posts.map((post, i) => {
            const canDelete = post.username === user.username
            ...
            const toolbar = (
              <div style={{
                ...
                {canDelete && (
                  <button onClick={() => handleDelete(post.id)} style={{ width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'none', border: 'none', cursor: 'pointer', fontSize: '13px', borderRadius: '6px', color: C.silver }}>🗑️</button>
                )}
              </div>
            )
            return (
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0, position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'baseline', gap: '7px', marginBottom: '4px' }}>
                      ...
                      <span style={{ color: C.silver, fontSize: '11px' }}>{communityTimeAgo(post.createdAt, lang)}</span>
                    </div>
                          ...
                          <div key={c.id} style={{ display: 'flex', gap: '8px', alignItems: 'flex-start' }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              <div style={{ display: 'flex', alignItems: 'baseline', gap: '6px', marginBottom: '3px' }}>
                                ...
                                <span style={{ color: C.silver, fontSize: '10.5px' }}>{communityTimeAgo(c.createdAt, lang)}</span>
                              </div>
                              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                                ...
                                {(c.username === user.username || user.role === 'admin') && (
                                  <button onClick={() => deleteComment(post.id, c.id)} style={{ background: 'none', border: 'none', color: C.silver, cursor: 'pointer', fontSize: '11px', flexShrink: 0 }}>✕</button>
                                )}
                              </div>
                              {pickerForComment === c.id && <ReactionPicker onPick={emoji => handleReactComment(post.id, c.id, emoji)} />}
                              <ReactionChips reactions={c.reactions} username={user.username} onToggle={emoji => handleReactComment(post.id, c.id, emoji)} />
                            </div>
                          </div>
                        ))}
...
export default function Dashboard({ initialUser }) {
  ...
  const breadcrumb = view === 'profile'
    ...
    : view === 'community' ? (lang === 'ar' ? 'مجتمع الطلاب' : 'Community')
      ...
      <div style={{ minHeight: '100vh', background: C.navy }}>
        ...
        <div className="dash-main" style={{ marginRight: desktopCollapsed ? '72px' : '240px', minHeight: '100vh', display: 'flex', flexDirection: 'column', transition: 'margin-right 0.2s' }}>
          ...
          <div className="dash-content-pad" style={{ flex: 1, padding: '28px 28px 40px' }}>
            ...
            {/* ── COMMUNITY VIEW ── */}
            {view === 'community' && (
              <CommunityView user={user} lang={lang} />
            )}

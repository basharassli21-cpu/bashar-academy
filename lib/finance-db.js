...
// Journal entries before this date are never voided automatically
const JOURNAL_CUTOFF = '2026-08-01'

// First fiscal year starts 2026-08-01; exported for use in reports
export const FISCAL_YEAR_START = '2026-08-01'
...
// ─── Journal Entry Voider ─────────────────────────────────────────────────
// Soft-deletes a journal entry and reverses its account balance adjustments.
// Only operates on entries dated >= JOURNAL_CUTOFF (2026-08-01).
async function voidJournalEntry(entryId) {
  if (!entryId) return
  const [entry] = await sql`
    SELECT id, entry_date FROM fin_journal_entries
    WHERE id = ${entryId} AND NOT is_deleted
      AND entry_date >= ${JOURNAL_CUTOFF}::date
  `
  if (!entry) return  // before cutoff, already deleted, or not found

  // Fetch lines with account type so we can reverse normal-balance deltas
  const lines = await sql`
    SELECT jl.account_id, jl.debit::numeric, jl.credit::numeric, fa.type
    FROM fin_journal_lines jl
    JOIN fin_accounts fa ON fa.id = jl.account_id
    WHERE jl.entry_id = ${entryId}
  `

  // Reverse each account balance: subtract what was originally added
  for (const line of lines) {
    const dr = parseFloat(line.debit)  || 0
    const cr = parseFloat(line.credit) || 0
    const isDebitNormal = ['asset','expense'].includes(line.type)
    const originalDelta = isDebitNormal ? (dr - cr) : (cr - dr)
    await sql`UPDATE fin_accounts SET balance = balance - ${originalDelta} WHERE id = ${line.account_id}`
  }

  // Soft-delete the entry (lines stay for audit; reports exclude via NOT je.is_deleted)
  await sql`UPDATE fin_journal_entries SET is_deleted = TRUE WHERE id = ${entryId}`
}

// Finds every journal entry whose source transaction was soft-deleted but the
// entry was never voided (e.g. deleted before the void-on-delete logic was deployed).
// Also directly voids any entry before JOURNAL_CUTOFF that is linked to a deleted source.
async function repairOrphanedJournalEntries() {
  const orphans = await sql`
    SELECT je.id
    FROM fin_journal_entries je
    WHERE NOT je.is_deleted
      AND (
        (je.source_type = 'expense' AND EXISTS (
          SELECT 1 FROM fin_expenses WHERE id = je.source_id AND is_deleted = TRUE
        )) OR
        (je.source_type = 'sale' AND EXISTS (
          SELECT 1 FROM fin_sales WHERE id = je.source_id AND is_deleted = TRUE
        )) OR
        (je.source_type = 'deposit' AND EXISTS (
          SELECT 1 FROM fin_deposits WHERE id = je.source_id AND is_deleted = TRUE
        )) OR
        (je.source_type IN ('withdrawal','withdrawal_paid') AND EXISTS (
          SELECT 1 FROM fin_withdrawals WHERE id = je.source_id AND is_deleted = TRUE
        ))
      )
  `
  for (const { id } of orphans) {
    await voidJournalEntry(id)
    // voidJournalEntry skips entries before JOURNAL_CUTOFF — ensure is_deleted regardless
    await sql`UPDATE fin_journal_entries SET is_deleted = TRUE WHERE id = ${id}`
  }
...
export async function getFinanceDashboard({ period = 'month', year, month, dateFrom, dateTo } = {}) {
  ...
  // Hard floor: fiscal year starts 2026-08-01 — never show data before it
  if (from < FISCAL_YEAR_START) from = FISCAL_YEAR_START
  ...
  const [prevRev] = await sql`
    ...
    WHERE NOT is_deleted AND payment_status NOT IN ('refunded','cancelled')
      ...
      AND sale_date >= ${FISCAL_YEAR_START}::date
  `
  const [exp] = await sql`
...
export async function softDeleteSale(id, actorUsername) {
  const [sale] = await sql`SELECT journal_entry_id FROM fin_sales WHERE id = ${id} AND NOT is_deleted`
  if (sale?.journal_entry_id) await voidJournalEntry(sale.journal_entry_id)
...
export async function softDeleteExpense(id, actorUsername) {
  const [exp] = await sql`SELECT journal_entry_id FROM fin_expenses WHERE id = ${id} AND NOT is_deleted`
  if (exp?.journal_entry_id) await voidJournalEntry(exp.journal_entry_id)
...
export async function updateWithdrawal(id, updates, actorUsername) {
  ...
  // Ensure journal_entry_id column exists on fin_withdrawals
  await sql`ALTER TABLE fin_withdrawals ADD COLUMN IF NOT EXISTS journal_entry_id INTEGER REFERENCES fin_journal_entries(id)`
  ...
  if (updates.status === 'paid' && before.status !== 'paid') {
    try {
      ...
      await sql`UPDATE fin_withdrawals SET journal_entry_id = ${entryId} WHERE id = ${id}`
    } catch (e) {
...
export async function softDeleteWithdrawal(id, actorUsername) {
  await sql`ALTER TABLE fin_withdrawals ADD COLUMN IF NOT EXISTS journal_entry_id INTEGER REFERENCES fin_journal_entries(id)`
  await sql`ALTER TABLE fin_withdrawals ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`
  const [wd] = await sql`SELECT journal_entry_id FROM fin_withdrawals WHERE id = ${id} AND NOT is_deleted`
  if (!wd) return
  if (wd.journal_entry_id) await voidJournalEntry(wd.journal_entry_id)
  await sql`UPDATE fin_withdrawals SET is_deleted = TRUE, deleted_at = NOW() WHERE id = ${id}`
  await writeFinAudit(actorUsername, 'admin', 'delete_withdrawal', 'withdrawal', id, null, null)
}
...
export async function getJournalEntries({ dateFrom = null, dateTo = null, sourceType = null, page = 1, limit = 50 } = {}) {
  await repairOrphanedJournalEntries()
  const offset = (page - 1) * limit
  const rows = await sql`
    SELECT je.*,
      COALESCE(
    ...
    WHERE NOT je.is_deleted
      AND je.entry_date >= ${FISCAL_YEAR_START}::date
...
export async function getPLReport({ dateFrom, dateTo } = {}) {
  ...
  const from = (dateFrom && dateFrom >= FISCAL_YEAR_START) ? dateFrom : fy.start
...
export async function getCashFlowReport({ year, month } = {}) {
  ...
  const [inflows, outflows] = await Promise.all([
    ...
    sql`SELECT expense_date::text AS date, total_amount::numeric AS amount, expense_number AS ref, category AS desc
        ...
        AND expense_date >= ${FISCAL_YEAR_START}::date
...
// ─── Balance Sheet ─────────────────────────────────────────────────────────
// Computes live from fin_journal_lines (not from stale fin_accounts.balance)
export async function getBalanceSheet({ asOfDate } = {}) {
  await repairOrphanedJournalEntries()
  const date = asOfDate || new Date().toISOString().split('T')[0]

  // For each account, sum debit/credit from all non-deleted journal entries up to asOfDate
  const accounts = await sql`
    SELECT
      fa.id, fa.code, fa.name, fa.type, fa.category,
    ...
    LEFT JOIN fin_journal_entries je
          ...
          AND je.entry_date BETWEEN ${FISCAL_YEAR_START}::date AND ${date}::date
    WHERE NOT fa.is_deleted
      AND fa.type IN ('asset','liability','equity')
    GROUP BY fa.id, fa.code, fa.name, fa.type, fa.category
    ORDER BY fa.code
  `

  // Retained earnings = revenue minus expenses for the current fiscal year up to asOfDate
  const fyStart = currentFiscalYear().start
  const [plRow] = await sql`
    SELECT
      COALESCE(SUM(CASE WHEN fa.type = 'revenue' THEN jl.credit - jl.debit ELSE 0 END), 0)::numeric AS revenue,
      COALESCE(SUM(CASE WHEN fa.type = 'expense' THEN jl.debit - jl.credit ELSE 0 END), 0)::numeric AS expenses
    FROM fin_journal_lines jl
    INNER JOIN fin_journal_entries je
            ON je.id = jl.entry_id AND NOT je.is_deleted
           AND je.entry_date BETWEEN ${fyStart}::date AND ${date}::date
    INNER JOIN fin_accounts fa ON fa.id = jl.account_id AND NOT fa.is_deleted
    WHERE fa.type IN ('revenue','expense')
  `

  const pf = v => parseFloat(v || 0)

  // Normal balance: asset/expense → DR side; liability/equity/revenue → CR side
  const balance = row => {
    const dr = pf(row.total_debit); const cr = pf(row.total_credit)
    return ['asset','expense'].includes(row.type) ? (dr - cr) : (cr - dr)
  }

  const assets      = accounts.filter(a => a.type === 'asset')
  const liabilities = accounts.filter(a => a.type === 'liability')
  const equity      = accounts.filter(a => a.type === 'equity')
  const retainedEarnings = pf(plRow.revenue) - pf(plRow.expenses)

  return {
    asOfDate: date,
    assets:      assets.map(a      => ({ code:a.code, name:a.name, balance:balance(a), category:a.category })),
    liabilities: liabilities.map(a => ({ code:a.code, name:a.name, balance:balance(a), category:a.category })),
    equity:      equity.map(a      => ({ code:a.code, name:a.name, balance:balance(a), category:a.category })),
    totalAssets:      assets.reduce((s, a) => s + balance(a), 0),
    totalLiabilities: liabilities.reduce((s, a) => s + balance(a), 0),
    totalEquity:      equity.reduce((s, a) => s + balance(a), 0) + retainedEarnings,
    retainedEarnings,
  }
}

// ─── Trial Balance ─────────────────────────────────────────────────────────
// Uses INNER JOIN subquery so date/deleted filters are applied to lines correctly
export async function getTrialBalance({ dateFrom, dateTo } = {}) {
  ...
  const rows = await sql`
    SELECT
      ...
      COALESCE(agg.total_debit,  0)::numeric AS total_debit,
      COALESCE(agg.total_credit, 0)::numeric AS total_credit
    FROM fin_accounts fa
    LEFT JOIN (
...
export async function getAccountStatement({ accountCode, dateFrom, dateTo } = {}) {
  const fy   = currentFiscalYear()
  // Never go before fiscal year start regardless of what is passed
  const from = (dateFrom && dateFrom >= FISCAL_YEAR_START) ? dateFrom : fy.start
  ...
  const [account] = await sql`
    ...
    FROM fin_accounts
    WHERE code = ${accountCode} AND NOT is_deleted
  `
  if (!account) return null

  const isDebitNormal = ['asset', 'expense'].includes(account.type)

  // Opening balance: entries within the fiscal year but before the period start
  // (entries before FISCAL_YEAR_START are always excluded — balance starts from zero)
  const [ob] = await sql`
    SELECT
  ...
  const openingBalance = isDebitNormal
    ? (parseFloat(ob.dr) - parseFloat(ob.cr))
    : (parseFloat(ob.cr) - parseFloat(ob.dr))
  ...
  const rows = lines.map(l => {
    ...
    return { ...l, debit: dr, credit: cr, running_balance: running }
  })
...
// ─── Admin: Rebuild & Resync ───────────────────────────────────────────────

// Recompute fin_accounts.balance from all non-deleted journal lines
export async function rebuildAccountBalances() {
  const sums = await sql`
    SELECT jl.account_id, fa.type,
      SUM(jl.debit)::numeric  AS total_debit,
      SUM(jl.credit)::numeric AS total_credit
    FROM fin_journal_lines jl
    INNER JOIN fin_journal_entries je ON je.id = jl.entry_id AND NOT je.is_deleted
    INNER JOIN fin_accounts fa ON fa.id = jl.account_id AND NOT fa.is_deleted
    GROUP BY jl.account_id, fa.type
  `
  // Reset all balances to 0
  await sql`UPDATE fin_accounts SET balance = 0 WHERE NOT is_deleted`
  ...
  for (const row of sums) {
    ...
    const newBalance = isDebitNormal ? (dr - cr) : (cr - dr)
    await sql`UPDATE fin_accounts SET balance = ${newBalance} WHERE id = ${row.account_id}`
    updated++
  }
  return { accountsUpdated: updated }
}

// Re-post journal lines for transactions that have a journal_entry_id but no lines
export async function resyncMissingJournals() {
  let synced = 0; const errors = []

  // Sales
  const sales = await sql`
    SELECT s.* FROM fin_sales s
    WHERE NOT s.is_deleted AND s.journal_entry_id IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM fin_journal_lines WHERE entry_id = s.journal_entry_id)
  `
  for (const sale of sales) {
    try {
      const paid = sale.payment_status === 'paid' ? parseFloat(sale.total) || 0
                 : parseFloat(sale.amount_paid) || 0
      await postJournalLines(sale.journal_entry_id, buildSaleLines({ ...sale, amount_paid: paid }))
      synced++
    } catch (e) { errors.push(`Sale ${sale.invoice_number}: ${e.message}`) }
  }

  // Expenses
  const expenses = await sql`
    SELECT e.* FROM fin_expenses e
    WHERE NOT e.is_deleted AND e.journal_entry_id IS NOT NULL
      AND NOT EXISTS (SELECT 1 FROM fin_journal_lines WHERE entry_id = e.journal_entry_id)
  `
  for (const exp of expenses) {
    try {
      await postJournalLines(exp.journal_entry_id, buildExpenseLines(exp))
      synced++
    } catch (e) { errors.push(`Expense ${exp.expense_number}: ${e.message}`) }
  }

  // After resyncing lines, rebuild balances to stay consistent
  const { accountsUpdated } = await rebuildAccountBalances()

  return { synced, accountsUpdated, errors }
}

export async function softDeleteDeposit(id, actorUsername) {
  await ensureDepositsTable()
  const [dep] = await sql`SELECT journal_entry_id FROM fin_deposits WHERE id = ${id}`
  if (dep?.journal_entry_id) await voidJournalEntry(dep.journal_entry_id)
  await sql`UPDATE fin_deposits SET is_deleted = TRUE, deleted_at = NOW() WHERE id = ${id}`
  await writeFinAudit(actorUsername, 'admin', 'delete_deposit', 'deposit', id, null, null)
}
...
async function ensureDepositsTable() {
  ...
  await sql`ALTER TABLE fin_deposits ADD COLUMN IF NOT EXISTS is_deleted BOOLEAN NOT NULL DEFAULT FALSE`
  await sql`ALTER TABLE fin_deposits ADD COLUMN IF NOT EXISTS deleted_at TIMESTAMPTZ`
}
...
export async function getDeposits({ page = 1, limit = 20 } = {}) {
  await ensureDepositsTable()
  const offset = (page - 1) * limit
  const [{ total }] = await sql`SELECT COUNT(*)::int AS total FROM fin_deposits WHERE NOT is_deleted`

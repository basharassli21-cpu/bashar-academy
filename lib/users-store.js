...
async function readUsers() {
  ...
  try {
    ...
    // Sort by newest first in case multiple blobs exist
    const sorted = [...blobs].sort((a, b) => new Date(b.uploadedAt) - new Date(a.uploadedAt))

    // Private blobs require Authorization header
    const res = await fetch(sorted[0].downloadUrl, {
      headers: { Authorization: `Bearer ${process.env.BLOB_READ_WRITE_TOKEN}` }
    })
    if (!res.ok) {
      console.error('[users-store] blob fetch failed:', res.status)
      return SEED_USERS
    }
    ...
    console.error('[users-store] readUsers error:', err.message)
    return SEED_USERS
  }
}
